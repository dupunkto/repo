...
epyrus-5_pkg_preinst() {
	gnome2_icon_savelist
}

epyrus-5_pkg_postinst() {
...
unsupported_compiler_warning() {
	ewarn "Building Epyrus with a compiler other than a supported gcc version"
	ewarn "may result in an unstable build."
	ewarn "Be aware that building Epyrus with an unsupported compiler"
...
unsupported_compiler_error() {
	eerror "Building Epyrus with a compiler other than a supported gcc version"
	...
	eerror "Be aware though that building Epyrus with an unsupported compiler"
...
set_pref() {
	echo "pref(\"$1\", $2);" >> "${S}/${obj_dir}/dist/bin/browser/defaults/preferences/epyrus.js"
}
